...
- [`development/ui/graph-rendering.md`](development/ui/graph-rendering.md) — the dependency graph's layout pipeline and rendering: module map, phase contracts, coordinate and edge-direction conventions, the DOM contract, and the viewport behaviour. ⚠️ Unlike the rest of `development/`, this currently documents *target* architecture for in-flight work (#172–#183) rather than current behaviour — its status table says what has actually landed.
...
- [`solution-proposals/haskell-graph-rendering.md`](solution-proposals/haskell-graph-rendering.md) — **decided**: replacing D3 with a layered (Sugiyama-style) graph-drawing pipeline computed in pure Haskell and rendered as server-side SVG, including the layout requirements derived from #169's reference images and an INVEST-shaped issue breakdown — implementation tracked in #172–#183, with the live development-facing reference in `development/ui/graph-rendering.md`.
