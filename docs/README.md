...
- [`solution-proposals/ci-cache-warming.md`](solution-proposals/ci-cache-warming.md) — **proposed, awaiting decision**: why every PR's first push and every merge-queue entry cold-builds despite an identical cache existing elsewhere (GitHub Actions cache scope, not a key mismatch — confirmed with real run data), recommending a push-to-`main`, build-only workflow to warm the cache — follow-up implementation tracked in #160, not started pending sign-off.
