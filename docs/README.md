...
- [`solution-proposals/haskell-graph-rendering.md`](solution-proposals/haskell-graph-rendering.md) — **proposed, not decided**: replacing D3 with a layered (Sugiyama-style) graph-drawing pipeline computed in pure Haskell and rendered as server-side SVG, including the layout requirements derived from #169's reference images and an INVEST-shaped issue breakdown to deliver it — spike only, nothing built.
