...
- [`architecture/graph-rendering.md`](architecture/graph-rendering.md) — the dependency graph's layout pipeline and rendering: module map, per-phase contracts and invariants, coordinate and edge-direction conventions, the DOM contract, viewport behaviour and the test plan. *In flight (#172–#183)* — its status table says what has actually landed.
