...
-- | Integration coverage for 'handlePutDescription' (#66), building on
  ...
  beforeWith resetBetweenTests $
    describe "handlePutDescription (integration)" $ do
      it "replaces the Node's description in the database" $ \pool -> do
        ...
        runSession
          ( do
              resp <- srequest $ putDescriptionRequest
                (fromSqlKey rootKey) (fromSqlKey projectKey) "UpdatedDescription"
...
putDescriptionRequest nodeId projectId descr = SRequest
