...
spec = do
  describe "validateConfig" $ do
    ...
    it "accumulates errors from multiple missing fields, not just the first" $ do
      let (result, errs) = runWriter (validateConfig validLookup
            { database' = Nothing
            , host'     = Nothing
            })
      result `shouldBe` Nothing
      errs `shouldBe`
        [ "DB_DATABASE is missing from environment config"
  ...
  describe "connStr" $
    it "assembles a libpq-style connection string from a DbConfig" $
      connStr validConfig `shouldBe`
        "host=localhost dbname=typeio user=typeio_user password=secret port=5432 sslmode=disable"
